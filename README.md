...
Use this commmand in the project directory, which in this case is the ELK directory, it is where the docker-compse.yml file is located# ThesisELK


# Start all the containers


Use this command in the project directory, which in this case is the ThesisELK directory.

    docker-compose up -d


To turn off the containers properly use 

    docker-compose down


To pause the specific containers.  Persistent data for Filebeat and Logstash will be kept between runs.

    docker-compose stop CONTAINER-NAME
    

To disable all paused containers, this will delete the persistent data for Logstash and Filebeat.

    docker-compose rm
    
# For the first run of the project

I have commented out the instructions to copy the files into the Filebeat and Logstash containers, so for the initial run-through of the project if one would want to load the data into Elasticsearch, open the docker-compose.yml file and go to the Filebeat and Logstash sections for further instructions. 
...
tshark -r PATH-TO-PCAP-FILE -T ek > FILE-TO-WRITE-TO.json
    ...
    When converting the files using this method keep in mind the problems associated with the conversion process
    which leads to the creation of invalid formatted files even if it is the official method of performing this action.
    The problems come in the form of multiple fields with the same name within the same event. 


Adding files

    The -H "Content-Type: application/x-ndjson" part is a safety precaution added in later versions of Elasticsearch
...
# Retrieving Files or Data 

Here is an example of retrieving data from the index logstash_ss7trace
    ...
    Where curlGet.txt is a file containing the query specifying which data fields to fetch from the specified index, this file is included in the project
    exportedTerms.json is the file to which the output is written to. 

For more in-depth explanations, tutorials and guides I refer you to these pages.
...
Then there is, of course, the official documentation on elastics websites
...
All patterns use some level of REGEX patterns. 
    ...
    For a complete list of all official Logstash filters : https://www.elastic.co/guide/en/logstash/current/filter-plugins.html
...
Example from the logstash_ss7trace.conf : 
    ...
    symbol to strong because Logstash does not know when to stop. 
...
On top of that is the fact that Logstash is very memory consuming as it is based on a JVM, making it so that it could be more beneficial 
...
where I check if a specific field exists before removing this. A possible way of doing it would be to check the file type being processed and using specific fields depending on that.

In the .conf files, I have two GROK filter fields, in which one of them just takes the entire message and stores it in the field log.original field. This is because of the problems
...
The big thing here is that the .yml files must be made by a superuser if created outside of the container and transferred into it.
...
superusers that are logged into the container, the problem persist and nothing i tried in regards to changing the permission in the filebeat container solved it. 
...
I will also mention that the filebeat .yml files are in a way similar to Python very strict when it comes to spacing and could easily cause problems if there every is to many or too few. 

Filebeat is able to read and handle multiple different types of files in a single file, and an example would be the filebeat_LOG_ss7trace.yml file, in there I have commented out a section in the filebeat.input
...
# JOINING INDEXES
     ...
     It is not normally possible in Elasticsearch it is one of its drawbacks. The only workaround is to format the data in the logs such that they could all be in the same index. 
     Now one can still perform queries on multiple indexes at once, but this can potentially get very complicated.
     
     If one wants to visualize the data however, then it is required that the timestamp field in the two different index patterns is named the same. If not, then Kibana becomes unable to handle the timestamp information between the fields at the same time.
     
         In this project, this is possible for the syslog and ss7trace files as they have the same name for the timestamp field. With the pcap files, however, this is not possible as the timestamp field is named something different from the other two log files. 
     
     A potential workaround is to create multiple different index patterns, perform an error search in one of them and try to limit the time frame to an acceptable level. Then go to another index pattern and use the same time frame. This will at least make it easier for the user to see which fields have a relation between each other in the aspect of time. 
     However, if there is another common field denominator between two index patterns, then you could combine them into a single index pattern, that is independent on time. And now, you have a relational database where you can search for certain fields and get all the results that satisfy the search terms. 
     
     Meaning if one wants to combine index patterns for the visualization tool in Kibana then one most first perform the overhead task of formatting the data sent to Elasticsearch such that there is at a shared field between the.



# Field Mapping
    
    For a complete of all the official field naming as per the Elastic Common Schema (ECS): https://www.elastic.co/guide/en/ecs/current/ecs-field-reference.html

# Custom Field Mapping description
    As there were certain field in the log files that I could not find a good representation for in ECS I had to create some custom fields.
    
    # ss7trace
    
    event.type                 : Contains the information regarding if the Event in question is a SENT, RECEIVED, LINKED.
    process.type                 : Is the process number
    ...
    **** Indicates the line in the ss7trace.log file, contains error code, see the Note.
    ...
    If one goes to the Dev Tool tab, then you get access to a console window where one can write and test out search queries and filters with the aid of autocomplete
        There is also a subwindow for Debugging GROK Patterns
...
# To solve a minor Time bug that you might encounter

    The bug is that while no changes are done to the data, because an incorrect Timezone is set Kibana will change will treat all of the timestamps as something else
...
# Docker
    ...
    This is no problem as their persistent data is stored in a docker volume specified in the docker-compose.yml file 
    
    # Move the persistent data between machines
    
    I admit to not having been able to figure this one out because of a lack of time, but it is possible to do. 
    With persistent data, I refer to all of the already stored logs in Elasticsearcha and the created Visualization in Kibana
